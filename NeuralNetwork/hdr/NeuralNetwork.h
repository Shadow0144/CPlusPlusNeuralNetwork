...
public:
	...
	void addConvolution1DLayer(size_t numKernels, const std::vector<size_t>& convolutionShape, const std::vector<size_t>& stride = { 1 }, const std::vector<size_t>& dilation = { 1 }, bool padded = false, bool addBias = false, ActivationFunctionType activationFunctionType = ActivationFunctionType::Identity, std::map<std::string, double> additionalParameters = std::map<std::string, double>());
	void addConvolution2DLayer(size_t numKernels, const std::vector<size_t>& convolutionShape, const std::vector<size_t>& stride = { 1 }, const std::vector<size_t>& dilation = { 1 }, bool padded = false, bool addBias = false, ActivationFunctionType activationFunctionType = ActivationFunctionType::Identity, std::map<std::string, double> additionalParameters = std::map<std::string, double>());
	void addConvolution3DLayer(size_t numKernels, const std::vector<size_t>& convolutionShape, const std::vector<size_t>& stride = { 1 }, const std::vector<size_t>& dilation = { 1 }, bool padded = false, bool addBias = false, ActivationFunctionType activationFunctionType = ActivationFunctionType::Identity, std::map<std::string, double> additionalParameters = std::map<std::string, double>());
	void addAveragePooling1DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
	void addAveragePooling2DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
	void addAveragePooling3DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
	void addMaxPooling1DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
	void addMaxPooling2DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
	void addMaxPooling3DLayer(const std::vector<size_t>& poolingShape, const std::vector<size_t>& stride = { }, bool hasChannels = true);
