...
MaxPoolingFunction::MaxPoolingFunction(size_t incomingUnits, size_t numUnits)
{
	this->hasBias = false;
...
xt::xarray<double> MaxPoolingFunction::feedForward(xt::xarray<double> inputs)
{
	auto dotProductResult = dotProduct(inputs);
	return lastOutput;
}

xt::xarray<double> MaxPoolingFunction::backPropagate(xt::xarray<double> sigmas)
{
	return denseBackpropagate(sigmas * activationDerivative());
}

xt::xarray<double> MaxPoolingFunction::activationDerivative()
{
	return (lastOutput > 0.0);
}

void MaxPoolingFunction::draw(ImDrawList* canvas, ImVec2 origin, double scale)
{
